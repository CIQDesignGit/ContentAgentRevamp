...
export function SectionSelectToggle({
  ...
  return (
    ...
    >
      {selected ? (
        <span className="flex size-4 items-center justify-center rounded-[3px] bg-slate-700">
...
export function BulkSelectControl({
  ...
  return (
    <div className="flex items-center justify-end pl-3">
      ...
      >
        <span className={cn("text-sm font-medium", noneSelected ? "text-slate-400" : "text-slate-500")}>
          {allSelected
            ? "Accept All"
            : noneSelected
            ? "Select all"
        ...
        {allSelected ? (
          <span className="flex size-4 items-center justify-center rounded-[3px] bg-slate-600">
            <Check className="size-3 stroke-3 text-white" />
          </span>
        ) : noneSelected ? (
